...
public:
    ...
    void deleteOfferSellByHash(const uint256 &hash);
...
private:
    ...
    void deleteOfferByHash(const std::string &tableName, const uint256 &hash);
    void deleteOldOffers(const std::string &tableName);
    ...
    int addOrEditOffer(const std::string &query, const OfferInfo &offer);
    int addOrEditMyOffer(const std::string &query, const MyOfferInfo &offer);
